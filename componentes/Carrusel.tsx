        ...
        <div className="overflow-hidden rounded-3xl bg-[#0A192F] elev-3">
          ...
          >
            ...
            <div className="order-1 flex flex-col justify-between p-8 text-white sm:p-12 lg:order-2 lg:p-14">
              <div>
                <div className="flex items-center gap-2 text-xs font-display font-bold tracking-[0.18em] text-brand-red-400 sm:text-sm">
                  <MonitorSmartphone size={16} />

                  {slide.category.toUpperCase()}
                </div>

                <h3 className="type-h3 mt-6 max-w-lg">{slide.title}</h3>
                ...
                <div className="flex gap-2">
                  {slides.map((item, index) => (
                    <button
                      ...
                      className={
                        index === activeSlide
                          ? "tap-target h-2 w-8 rounded-full bg-brand-red-400 transition-all"
                          : "tap-target h-2 w-2 rounded-full bg-white/30 transition-colors hover:bg-white/60"
                      }
                    />
                  ))}
                </div>
