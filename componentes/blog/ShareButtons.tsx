...
export default function ShareButtons({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="mr-1 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Compartir</span>
      {links.map((link) => (
        <a
          key={link.label}
          href={link.href}
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 dark:border-white/15 text-slate-500 dark:text-slate-400 transition hover:border-[#E63946] hover:text-[#E63946]"
        >
          <link.icon size={16} />
        </a>
      ))}
