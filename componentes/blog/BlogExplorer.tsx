...
export type SearchablePost = ArticleCardData & { tags: string[]; searchIndex: string };
...
export default function BlogExplorer({
  ...
  return (
    <div>
      <BlogSearch value={query} onChange={setQuery} resultsCount={query ? filtered.length : undefined} />

      <div className="mt-14">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-[11px] font-bold tracking-[0.18em] text-[#E63946] uppercase">Categorías</p>
      ...
      <div id="articulos" className="mt-20 scroll-mt-24">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-display font-bold tracking-tight text-[#07182D] dark:text-white sm:text-4xl">
              {activeCategory ? `Artículos sobre ${activeCategory}` : "Biblioteca completa."}
            </h2>
          </div>
          {activeCategory && (
            <button
              ...
              className="rounded-xl border border-slate-200 dark:border-white/15 px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:border-[#E63946] hover:text-[#E63946] transition"
            >
              Quitar filtro
            </button>
          )}
        ...
        {filtered.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-12 flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 dark:border-white/15 py-20 text-center"
          >
            <SearchX size={32} className="text-slate-300 dark:text-slate-600" />
            <p className="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">
              No encontramos artículos con esos criterios. Prueba con otra búsqueda o quita el filtro de categoría.
            </p>
          </motion.div>
        ) : (
